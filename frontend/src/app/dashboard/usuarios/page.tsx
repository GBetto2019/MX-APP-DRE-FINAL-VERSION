...
export default function UsuariosPage() {
  ...
  async function criarUsuario(e: React.FormEvent) {
    ...
    try {
      ...
      setErro(err instanceof Error ? err.message : 'Erro ao criar usuÃ¡rio')
    } finally {
      setCriando(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-mx-navy">GestÃ£o de UsuÃ¡rios</h1>

      <form onSubmit={criarUsuario}
        className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="mb-4 font-semibold text-gray-800">Novo UsuÃ¡rio</h2>
        ...
        <button type="submit" disabled={criando}
          ...
          {criando ? 'Criandoâ€¦' : 'Criar UsuÃ¡rio'}
        </button>
      </form>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-left">
                ...
                <th className="px-5 py-3 font-medium text-gray-600">AÃ§Ãµes</th>
              </tr>
            </thead>
            <tbody>
              {usuarios.map((u) => (
              ...
              {usuarios.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-gray-400">Nenhum usuÃ¡rio encontrado</td></tr>
              )}
            </tbody>
          </table>
        )}
